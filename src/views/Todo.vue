      ...
      <div
        ...
        :style="{
          'background-color':
            ...
            todoData.todo[indexCategory].data.length ? 'red' : ''
        }"
      >
...
const openCat = (index) => {
  ...
  if (openCategory.value[index].offsetHeight == 65) {
    openCategory.value[index].style.height =
      openCategory.value[index].children[0].scrollHeight + 20 + "px";
...
const projectsEditForSearch = (type, index, todoData = "") => {
  ...
  if (type == "add") {
  ...
  } else if (type == "addInMiddle") {
    storeTodos.todos[route.params.id - 1].todo[
      categorySelected.value
    ].data.splice(index, 0, todoData);
  } else if (type == "delete") {
    storeTodos.todos[route.params.id - 1].todo[
      categorySelected.value
    ].data.splice(index, 1);
    ...
    storeTodos.todos[route.params.id - 1].todo[categorySelected.value].name =
      todoData;
...
const editTodo = async (categoryIndex, dataIndex) => {
  ...
  /*
storeTodos.todos[route.params.id].todo[categorySelected.value].data[index] =
      todoData;
 storeTodos.todos[route.params.id].todo[categorySelected.value].data.splice(
      index,
      0,
      todoData
    );
